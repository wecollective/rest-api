...
io.on('connection', socket => {
    ...
    socket.on('outgoing-signal-request', payload => {
        io.to(payload.userToSignal).emit('incoming-signal-request', { signal: payload.signal, userSignaling: payload.userSignaling })
    })

    socket.on('outgoing-signal', payload => {
        io.to(payload.userToSignal).emit('incoming-signal', { signal: payload.signal, id: socket.id })
    })
