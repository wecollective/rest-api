                ...
                (space) =>
                    new Promise(async (resolve) => {
                        const newSpace = await Space.create({
                            creatorId,
                            handle: `${spaceHandlePrefix}-${
                                space.handle || space.name.toLowerCase().replace(/[^a-z0-9]/g, '-')
                            }`,
                            name: space.name,
            ...
            posts.map(
                (post) =>
                    new Promise(async (resolve) => {
                        ...
                        const totalComments =
                            comments.filter((c) => c.postId === post.id).length - 1
                ...
                for (let i = 0; i < replies.length; i += 1) {
                    ...
                    if (post) {
                        const parent = replies.find(
                            (r) =>
                                r.postId === post.id &&
                                r.commentNumber === replies[i].replyToCommentNumber
                        )
...
// // todo: test if promises wrapped in functions wait until fired to run
// const order = []
...
// // const tasks = [a, b, c]
// // for (const task of tasks) await task
// Promise.all([a, b, c]).then(() => res.status(200).json(order))

// function addReplies(comment, postId) {
//     comment.replies = comments.filter(
//         (c) => c.replyToCommentNumber === comment.commentNumber && c.postId === postId
//     )
//     if (comment.replies.length) comment.replies.forEach((reply) => addReplies(reply))
// }

// // map comments into json
// const json = posts
//     .filter((p) => p.title === 'Proposal to dissolve forum')
//     .map((post) => {
//         const postComments = comments.filter(
//             (c) => c.postId === post.id && !c.replyToCommentNumber
//         )
//         postComments.forEach((comment) => addReplies(comment, post.id))
//         return {
//             ...post,
//             comments: postComments,
//         }
//     })
// json.sort((a, b) => a.comments.length - b.comments.length)
